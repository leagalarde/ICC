  ...
  public function indexAdmin(){
    ...
    //insert new notif in notification_tbl
        ...
        foreach($pr as $pr) {
            ...
            $days = date_diff(date_create($pr->proj_start_date),date_create("now"))->format('%r%a days') + 1;
...
//***** PROJECT_ADD *****//
    ...
    $client = DB::table('client_rep_tbl')->where('cr_delete',0)->get();
  ...
  public function getClientDetails(Request $req){
    $type = DB::table('client_rep_tbl')->where('cr_id',$req->cr_id)->get();
    return response()->json($type);
  } 

  public function getCompanyDetails(Request $req){
    $type = DB::table('client_tbl')->where('cl_no',$req->cl_no)->get();
    return response()->json($type);
  }
...
public function ProjectEdit(){
   ...
   //notification
    	...
    	return view('project_edit',['id' => $id, 'notif' => $notif, 'notifcount' => $notifcount, 'nvc' => $nvc, 'invoice' => $invoice, 'proj' => $proj, 'client' => $client, 'equipdep' => $equipdep, 'plan' => $plan, 'task' => $task, 'PM' => $PM, 'phase' => $phase, 'contract' => $contract, 'ter' => $ter,'equipdep'=>$equipdep,'equipreq'=>$equipreq,'empPic'=>$empPic]);
    }
    ...
    public function editprojinvoice(){
		...
		if($payment_status=="Paid" && $recentinstat=="Sent"){
		...
		} else if($payment_status=="Sent" && $recentinstat=="Paid"){
			$newpaid = $paid - $invoice_amount;
			$newbalance = $balance + $invoice_amount;
			DB::table('project_tbl as pr')
				->join('contract_info_tbl as ci','pr.ci_no','=','ci.ci_no')
            ...
            DB::table('payment_tbl')->where('invoice_no', $_POST['invoice_no'])->delete();
		} else if ($payment_status=="Sent" && $recentinstat=="Sent"){
			
		} else if ($payment_status=="Paid" && $recentinstat=="Paid"){
	...
	public function getinvoice(Request $req){
		$type = DB::table('invoice_tbl')
			...
			->join('project_info_tbl as pi','pi.proj_no','=','pr.proj_no')
			->where('invoice_tbl.invoice_id',$req->id)->get();
